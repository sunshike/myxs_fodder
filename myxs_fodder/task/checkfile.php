...
require_once '../../../framework/function/file.func.php';
require_once '../../../framework/function/cache.func.php';
require_once '../../../framework/function/compat.biz.func.php';
...
$content_count = intval(pdo_fetchcolumn("select count(*) from " . tablename('myxs_fodder_content') . " where uniacid=:uniacid and type='img' and content_status in (0,1,2) and is_check=0  ",array(':uniacid'=>$uniacid)));
$start = max(1, intval($_GPC['s']));
$end = 1;

if($content_count > 0){
    run($start,$end,$content_count,$uniacid);
}
...
function run($start,$end,$total,$uniacid){
    global $_GPC, $_W;

    $system = pdo_fetch("select * from " . tablename('myxs_fodder_system') . " where uniacid = :uniacid and system_code = :system_code", array(':uniacid'=>$uniacid,':system_code'=>'system'));
    $system_content  = json_decode($system['system_content'],true);
    $wg_img = $system_content['wg_img'];
    if(empty($wg_img)){
        $wg_img = 'https://'.$_SERVER['HTTP_HOST'].'/addons/myxs_fodder/img/wg.png';
    }

    $content = pdo_fetchall("select content,member_id,content_id from " . tablename('myxs_fodder_content') . " where uniacid=:uniacid and type='img' and content_status in (0,1,2) and is_check=0 order by content_id asc,content_status asc LIMIT ".($start - 1) * $end ."," . $end,array(':uniacid'=>$uniacid));
    if(!empty($content)){
        $aaa =array();
        foreach ($content as $key=>$value){
            ...
            foreach ($content[$key]['content'] as $keys => $values){
                ...
                $new_img = getThumb($filePath,400,400);
        ...
        foreach ($aaa as $k=>$v){
            ...
            $data['content'] = $v['c'];
            $data['content2'] = $v['d'];
            pdo_update('myxs_fodder_content',$data,array('content_id'=>$v['id'],'uniacid'=>$uniacid));
        }

    }

    header("Location: https://".$_SERVER['HTTP_HOST']."/addons/myxs_fodder/task/checkfile.php?i=".$uniacid);
